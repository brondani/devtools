...
ProjMgrExtGenerator::ProjMgrExtGenerator(ProjMgrParser* parser) : m_checkSchema(false) {
  m_parser = parser;
}
...
const string& ProjMgrExtGenerator::GetGlobalGenDir(const string& generatorId) {
  return(m_globalGenerators[generatorId].output);
}
...
bool ProjMgrExtGenerator::GetCgen(const string& contextId, ClayerItem& cgen) {
  ContextName context;
  ...
  for (const auto& [generator, genDirs] : GetUsedGenerators()) {
    for (const auto& [genDir, contexts] : genDirs) {
      if (find(contexts.begin(), contexts.end(), contextId) != contexts.end()) {
        ...
        cgen = m_parser->GetClayers().at(cgenFile);
        return true;
      }
    }
  }
